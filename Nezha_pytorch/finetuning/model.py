...
class BertForClass_MultiDropout(nn.Module):
    def __init__(self, config):
        ...
        self.multi_dropouts = nn.ModuleList(
            [nn.Dropout(config.dropout) for _ in range(self.multi_drop)])
        self.classifier = nn.Linear(
            self.bert_config.hidden_size * 2, self.n_classes)
...
class BertLastTwoCls(nn.Module):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(p=config.dropout)
        self.classifier = nn.Linear(
            self.bert_config.hidden_size, config.num_class)
...
class BertLastTwoClsPooler(nn.Module):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(p=config.dropout)
        self.classifier = nn.Linear(
            self.bert_config.hidden_size * 3, config.num_class)
...
class BertLastTwoEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(p=config.dropout)
        self.classifier = nn.Linear(
            self.bert_config.hidden_size * 2, config.num_class)
...
class BertLastFourCls(nn.Module):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(p=config.dropout)
        self.classifier = nn.Linear(
            self.bert_config.hidden_size * 4, config.num_class)
...
class BertLastFourClsPooler(nn.Module):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(p=config.dropout)
        self.classifier = nn.Linear(
            self.bert_config.hidden_size * 5, config.num_class)
...
class BertDynCls(nn.Module):
    ...
    def forward(self, input_ids, input_masks, segment_ids):
        ...
        concat_out = concat_out.reshape(
            batch_size, -1, self.bert_config.hidden_size)
...
class BertRNN(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc_rnn = nn.Linear(
            self.hidden_dim * self.num_directions, config.num_class)
...
class BertCNN(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc_cnn = nn.Linear(
            self.num_filters * len(self.filter_sizes), config.num_class)
...
class Electra(nn.Module):
    ...
    def __init__(self, config):
        ...
        config_json = 'bert_config.json' if os.path.exists(
            config.model_path + 'bert_config.json') else 'config.json'
        self.electra_config = CONFIGS[config.model].from_pretrained(
