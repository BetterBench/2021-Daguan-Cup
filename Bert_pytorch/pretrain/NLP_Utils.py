...
if not os.path.exists(train_clean):
    ...
    test_df["text"] = test_df["text"].progress_apply(
        lambda x: preprocess_text(x))
...
class MLM_Data(Dataset):
    ...
    def random_mask(self, text_ids):
        ...
        idx = 0
        while idx < len(rands):
            if rands[idx] < 0.3:  # 需要mask
                # 若要mask，进行x_gram mask的概率
    ...
    def __getitem__(self, item):
        ...
        input_ids = [self.tk.cls_token_id] + \
            text1_ids + [self.tk.sep_token_id]  # 拼接
...
def unionList(ls): return list(chain(*ls))  # 按元素拼接


def splitList(x, bs): return [x[i:i+bs] for i in range(0, len(x), bs)]  # 按bs切分
...
def blockShuffle(data: list, bs: int, sortBsNum, key):
    ...
    # 为None就是整体排序
    sortBsNum = len(data)//bs if sortBsNum is None else sortBsNum
...
class blockShuffleDataLoader(DataLoader):
    ...
    def __iter__(self):
        #分块shuffle
        self.dataset.data = blockShuffle(
            self.dataset.data, self.batch_size, self.sortBsNum, self.key)
