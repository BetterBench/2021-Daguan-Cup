...
maxlen=100
batch_size=32
vocab_file_dir = 'Bert_pytorch/pretrain/bert_model/vocab_3462.txt'
...
# 把层数改为8层
# 必须是绝对路径
model = BertForMaskedLM.from_pretrained("Bert_pytorch/bert_model_1000/")  # './bert-base-chinese')
...
training_args = TrainingArguments(
    output_dir='Bert_pytorch/bert_output',#必须是绝对路径
    overwrite_output_dir=True,
    num_train_epochs=1000,
    per_device_train_batch_size=batch_size,
    save_steps=1000,#每50个epoch save一次
    save_total_limit=3,
...
if __name__ == '__main__':
    trainer.train()
    trainer.save_model('./bert_model_3462')
